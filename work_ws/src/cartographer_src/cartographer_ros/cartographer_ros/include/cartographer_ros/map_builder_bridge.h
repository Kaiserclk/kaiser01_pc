...
#include "geometry_msgs/msg/pose.hpp"//swx
...
class MapBuilderBridge {
 public:
  ...
  //swx
  // Submits a relocalization attempt for 'initial_pose' to the pose graph.
  // Returns the relocalization epoch of the new attempt, or 0 if the
  // request was rejected (e.g. no active localization trajectory).
  int StartRelocalization(const bool use_initial_pose,
                          const int trajectory_id,
                          const ::cartographer::transform::Rigid3d& initial_pose);

  //swx
  // Score of the most recent relocalization match (0 while pending/failed).
  double GetRelocalizationScore();
  //swx
  // Map-frame 2D pose of the best relocalization match, or nullopt.
  ::cartographer::transform::Rigid3d GetRelocalizationPose();

  //swx
  // Returns the local-to-global (odom→map) transform for 'trajectory_id' as
  // currently known to the pose graph.
  ::cartographer::transform::Rigid3d GetLocalToGlobalTransform(
      const int trajectory_id);
  //swx
  // Returns the global pose of 'trajectory_id' interpolated at 'time' — the
  // same function ComputeLocalToGlobalTransform uses internally.
  ::cartographer::transform::Rigid3d GetInterpolatedGlobalTrajectoryPose(
      const int trajectory_id,
