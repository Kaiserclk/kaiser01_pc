...
#include "cartographer_ros_msgs/msg/robot_pose.hpp"//swx
#include "cartographer_ros_msgs/msg/relocalization_status.hpp"//swx
...
//swx
 ...
 private:
  ...
  //swx
  void PublishRelocalizationStatus(const std::string& status,
                                   const double score, const int epoch);
  // Polls the scan-matching result for the pending relocalization request.
  void CheckRelocalizationResult();
  // Restarts the active localization trajectory at 'map_pose' using the
  // standard initial-trajectory-pose mechanism.
  void RestartLocalizationTrajectory(
      const ::cartographer::transform::Rigid3d& map_pose);
  ...
  ::rclcpp::TimerBase::SharedPtr maybe_warn_about_topic_mismatch_timer_;
    ::rclcpp::TimerBase::SharedPtr pub_robot_pose_timer_;//swx


  //swx
  ::rclcpp::Subscription<::geometry_msgs::msg::PoseWithCovarianceStamped>::SharedPtr initial_pose_subscriber_;
  ::rclcpp::Publisher<::cartographer_ros_msgs::msg::RobotPose>::SharedPtr current_pose_publisher_;

  //swx
  ::rclcpp::Publisher<::cartographer_ros_msgs::msg::RelocalizationStatus>::SharedPtr relocalization_status_publisher_;
  ::rclcpp::TimerBase::SharedPtr relocalization_check_timer_;
  bool relocalization_pending_ = false;
  int relocalization_epoch_ = 0;
  std::chrono::steady_clock::time_point relocalization_deadline_;
  ::cartographer::transform::Rigid3d relocalization_fallback_pose_;
