...
//swx
int MapBuilderBridge::StartRelocalization(
    const bool use_initial_pose, const int trajectory_id,
    const ::cartographer::transform::Rigid3d& initial_pose) {
  if (!map_builder_->pose_graph()->LocalizeOKagvPoses(use_initial_pose,
                                                     trajectory_id,
                                                     initial_pose)) {
    LOG(WARNING) << "Relocalization request rejected by the pose graph.";
    return 0;
  }
  // The pose graph increments its epoch synchronously when the request is
  // scheduled, so this is the epoch of the attempt we just submitted.
  return map_builder_->pose_graph()->GetRelocalizationEpoch();
}

//swx
double MapBuilderBridge::GetRelocalizationScore() {
  return map_builder_->pose_graph()->GetRelocalizationScore();
}

//swx
Rigid3d MapBuilderBridge::GetRelocalizationPose() {
  const auto pose_2d =
      map_builder_->pose_graph()->GetRelocalizationPose();
  if (!pose_2d.has_value()) {
    return Rigid3d::Identity();
  }
  return ::cartographer::transform::Embed3D(pose_2d.value());
}

//swx
Rigid3d MapBuilderBridge::GetLocalToGlobalTransform(
    const int trajectory_id) {
  return map_builder_->pose_graph()->GetLocalToGlobalTransform(trajectory_id);
}

//swx
Rigid3d MapBuilderBridge::GetInterpolatedGlobalTrajectoryPose(
    const int trajectory_id,
    const ::cartographer::common::Time time) const {
  return map_builder_->pose_graph()->GetInterpolatedGlobalTrajectoryPose(
      trajectory_id, time);
}
    ...
    rclcpp::Time node_time)
{
  ...
  //LOG(INFO) << "landmark pose"<<landmark_poses_list.markers.size();
