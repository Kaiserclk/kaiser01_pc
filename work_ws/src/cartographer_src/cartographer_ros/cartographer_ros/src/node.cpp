...
//swx: used by the relocalization fallback to restart the localization
// trajectory with the same configuration the node was started with. The
// flags are defined in node_main.cpp and read via the gflags registry here
// to avoid linking against the definition symbols.
...
//swx
// Fall back to restarting the trajectory at the requested pose when no
// candidate submap matches within this window.
constexpr double kRelocalizationTimeoutSec = 3.;
constexpr int kRelocalizationCheckPeriodMs = 100;
    ...
    : node_options_(node_options)
{
  ...
  scan_matched_point_cloud_publisher_ =
      ...
      //swx 
 initial_pose_subscriber_=   node_->create_subscription<::geometry_msgs::msg::PoseWithCovarianceStamped>(
     "/initialpose", 1,std::bind(&Node::HandleInitialPoseMessage,this,std::placeholders::_1));   

   //swx
    current_pose_publisher_ = node_->create_publisher<cartographer_ros_msgs::msg::RobotPose>(
        kRobotPoseTopic, 30);//swxlog

  //swx
  relocalization_status_publisher_ =
      node_->create_publisher<cartographer_ros_msgs::msg::RelocalizationStatus>(
          kRelocalizationStatusTopic, 10);
  relocalization_check_timer_ = node_->create_wall_timer(
      std::chrono::milliseconds(kRelocalizationCheckPeriodMs),
      [this]() { CheckRelocalizationResult(); });
...
//swx
  ...
  constrain_list_timer_ = node_->create_wall_timer(
    ...
    //swx
    //hz == 33
    pub_robot_pose_timer_ = node_->create_wall_timer(
    std::chrono::milliseconds(int(node_options_.trajectory_publish_period_sec * 1000)),
    [this]() {
      PublishRobotPose();
    });//100hz
      ...
      if (node_options_.publish_to_tf) {
        if (trajectory_data.trajectory_options.provide_odom_frame) {
          ...
          tf_broadcaster_->sendTransform(stamped_transform);
                ...
                //swx
          ::geometry_msgs::msg::Transform temp = stamped_transform.transform;
          //swxchanged
          cartographer_ros_msgs::msg::RobotPose current_pose;
          ...
          // if(trajectory_data.covariance_score > 0.5)
          // {
          //   current_pose.covariance_score =  trajectory_data.covariance_score;
          // }
          // else if(trajectory_data.covariance_score > 0)
          // {
          //   current_pose.covariance_score = 0.5;
          // }
          // else
          // {
...
void Node::HandleLandmarkMessage(
  ...
  LOG(INFO) << "HandleLandmarkMessage ";
...
void Node::HandleInitialPoseMessage(
  ...
  if (!initial_pose.IsValid()) {
    LOG(ERROR) << "Relocalization rejected: the /initialpose quaternion is "
                  "not normalized.";
    PublishRelocalizationStatus("FAILED", 0., 0);
    return;
  }

  // Use the current scan and the candidate map submaps to refine the RViz
  // estimate. This adds a scan-matching constraint rather than merely applying
  // the requested pose as a new trajectory origin.
  const int epoch = map_builder_bridge_->StartRelocalization(
      true /* use_initial_pose */, last_map_trajectory_id, initial_pose);
  if (epoch <= 0) {
    PublishRelocalizationStatus("FAILED", 0., 0);
    return;
  }

  relocalization_epoch_ = epoch;
  relocalization_pending_ = true;
  relocalization_fallback_pose_ = initial_pose;
  relocalization_deadline_ =
      std::chrono::steady_clock::now() +
      std::chrono::duration_cast<std::chrono::steady_clock::duration>(
          std::chrono::duration<double>(kRelocalizationTimeoutSec));
  PublishRelocalizationStatus("REQUEST_ACCEPTED", 0., epoch);
}

//swx
void Node::PublishRelocalizationStatus(const std::string& status,
                                       const double score, const int epoch) {
  cartographer_ros_msgs::msg::RelocalizationStatus msg;
  msg.header.stamp = node_->now();
  msg.status = status;
  msg.score = score;
  msg.epoch = epoch;
  relocalization_status_publisher_->publish(msg);
  LOG(INFO) << "Relocalization status: " << status << " (score " << score
            << ", epoch " << epoch << ")";
}

void Node::CheckRelocalizationResult() {
  if (!relocalization_pending_) {
    return;
  }

  const double score = map_builder_bridge_->GetRelocalizationScore();
  if (score > 0.) {
    relocalization_pending_ = false;
    const ::cartographer::transform::Rigid3d matched_pose =
        map_builder_bridge_->GetRelocalizationPose();
    LOG(INFO) << "Relocalization succeeded with scan-matching score " << score
              << "; restarting localization trajectory at the matched pose.";
    PublishRelocalizationStatus("SUCCESS", score, relocalization_epoch_);
    RestartLocalizationTrajectory(matched_pose);
    return;
  }

  // No candidate submap matched within the timeout: apply the requested pose
  // directly so the robot still jumps to the RViz estimate.
  if (std::chrono::steady_clock::now() >= relocalization_deadline_) {
    relocalization_pending_ = false;
    LOG(WARNING) << "Relocalization failed (no matching submap within "
                 << kRelocalizationTimeoutSec
                 << "s); applying the requested initial pose as fallback.";
    PublishRelocalizationStatus("FALLBACK_INITIAL_POSE", 0.,
                                 relocalization_epoch_);
    RestartLocalizationTrajectory(relocalization_fallback_pose_);
  }
}

//swx
// Restarts the active localization trajectory at 'map_pose' so that the
// scan-matched pose takes immediate effect, regardless of how many nodes the
// previous trajectory had accumulated.
void Node::RestartLocalizationTrajectory(const Rigid3d& map_pose) {
  google::CommandLineFlagInfo info;
  std::string configuration_directory;
  std::string configuration_basename;
  if (!google::GetCommandLineFlagInfo("configuration_directory", &info)) {
    LOG(ERROR) << "Relocalization restart failed: configuration_directory "
                  "flag is not registered.";
    return;
  }
  configuration_directory = info.current_value;
  if (!google::GetCommandLineFlagInfo("configuration_basename", &info)) {
    LOG(ERROR) << "Relocalization restart failed: configuration_basename "
                  "flag is not registered.";
    return;
  }
  configuration_basename = info.current_value;
  if (configuration_directory.empty() || configuration_basename.empty()) {
    LOG(ERROR) << "Relocalization restart failed: the node was not started "
                  "with -configuration_directory/-configuration_basename.";
    return;
  }
  ...
  for (const auto& entry : trajectory_states) {
    if (entry.first != last_map_trajectory_id &&
        entry.second == TrajectoryState::ACTIVE) {
      active_trajectory_id = entry.first;
    }
  }
  if (active_trajectory_id < 0) {
    LOG(ERROR) << "Relocalization restart failed: no active localization "
                  "trajectory.";
    return;
  }

  const auto now = ::cartographer_ros::FromRos(node_->now());
  const Rigid3d map_interpolated_pose =
      map_builder_bridge_->GetInterpolatedGlobalTrajectoryPose(
          last_map_trajectory_id, now);
  const Rigid3d relative_pose = map_interpolated_pose.inverse() * map_pose;

  ::cartographer::mapping::proto::InitialTrajectoryPose
      initial_trajectory_pose;
  initial_trajectory_pose.set_to_trajectory_id(last_map_trajectory_id);
  *initial_trajectory_pose.mutable_relative_pose() =
      ::cartographer::transform::ToProto(relative_pose);
  initial_trajectory_pose.set_timestamp(::cartographer::common::ToUniversal(
      ::cartographer_ros::FromRos(node_->now())));
  *trajectory_options.trajectory_builder_options
       .mutable_initial_trajectory_pose() = initial_trajectory_pose;

  if (!ValidateTrajectoryOptions(trajectory_options)) {
    LOG(ERROR) << "Relocalization restart failed: invalid trajectory options.";
    return;
  }
  const auto finish_status = FinishTrajectoryUnderLock(active_trajectory_id);
  if (finish_status.code != cartographer_ros_msgs::msg::StatusCode::OK) {
    LOG(ERROR) << "Relocalization restart failed: " << finish_status.message;
    return;
  }
  if (!ValidateTopicNames(trajectory_options)) {
    LOG(ERROR) << "Relocalization restart failed: replacement trajectory "
                  "topics are still in use.";
    return;
  }
